...
#### Maura Behavioural Data (Response scores) ####

Behav_d <- read.table("/Users/patrikpluchino/Dropbox/Condivisa - Maura/Maura_Data_R_New/Maura_Behavioural_no_sj6.txt", header = T)
str(Behav_d)
...
# Subset of data in order to consider just the Tot Response Scores
...
data:  Behav_R3$Score
W = 0.7961, p-value = 6.264e-11

# Quick plot of Score to visually inspect the data
qplot(Behav_R3$Score)

# Anova: DV = Response Scores & IV = Type of Stimulus 
...
# Mean scores values as a function of Stimulus 
...
pairwise.t.test(Behav_R3$Score, Behav_R3$Stimulus, paired = T, p.adjust.method = "bonferroni")
...
# Repeated Measures ANOVA - DV = Response Scores , IV = DISPOSITION & NUMEROSITY 
